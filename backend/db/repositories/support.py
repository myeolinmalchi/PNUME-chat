    ...
    def search_supports_hybrid(
        ...
        **kwargs
        ...
        rank_content = (
            ...
            ).join(SupportChunkModel,
                   SupportModel.id == SupportChunkModel.support_id).group_by(SupportModel.id
        ...
        query = (
            self.session.query(SupportModel,
                               rrf_score).join(rank_content, SupportModel.id == rank_content.c.id
                                               ).join(rank_title,
                                                      SupportModel.id == rank_title.c.id).order_by(rrf_score.desc()
                                                                                                   ).limit(k)
        )
